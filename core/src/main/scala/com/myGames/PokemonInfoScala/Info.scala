...
object Info {
  ...
  lazy val enemyPokemon: List[Pokemon] = List(
    Pokemon("Rattata", Type.Normal, List(Some(allMoves("Tackle")), None, None, None)),
    Pokemon("Budew", Type.Grass, List(Some(allMoves("Tackle")), None, None, None)),
    Pokemon("Starly", Type.Flying, List(Some(allMoves("Tackle")), None, None, None))
  )
